...
DEFAULT_INPUT = "data/yale_courses.json"
DEFAULT_OUTPUT = "data/YALE_coursecatalogstructured.json"
...
def transform_entry(entry: Dict[str, Any]) -> Dict[str, Any]:
    ...
    description = clean_text(entry.get("Course Descrpition"))
...
def main() -> None:
    ...
    if not isinstance(raw_data, list):
        raise ValueError("Yale catalog input must be a list of courses.")

    structured = [transform_entry(entry) for entry in raw_data]
    write_json(output_path, structured)
    print(f"Processed {len(structured)} Yale courses -> {output_path}")
