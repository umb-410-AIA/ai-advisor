'''
UPenn Course Catalog Scraper: Simply scrapes the UPenn course catalog and outputs data into a JSON file
...
def scrape_upenn(progress_position: Optional[int] = None):  # Codex-added: position for nested tqdm.
    ...
    tqdm_kwargs = {"desc": "UPenn subjects", "unit": "dept", "leave": False}
    ...
    for subject in tqdm(subjectLinks, **tqdm_kwargs):  # Codex-added: visualize subject scraping progress.
        ...
        for courseBlock in courseBlocks:
            courseBlockTitle = courseBlock.find('p', class_="courseblocktitle")
            courseName = courseBlockTitle.get_text(" ", strip=True)
            courseBlockExtras = courseBlock.find_all('p', class_="courseblockextra")

            courseDescription = ""
            courseCredits = ""

            for extra in courseBlockExtras:
                text = extra.get_text(" ", strip=True)
                lower_text = text.lower()
                if "course units" in lower_text:
                    courseCredits += " " + text
                else:
                    courseDescription += " " + text
            data.append({
                "Course Name": courseName,
                "Course Descrition": courseDescription,
...
if __name__ == "__main__":
    courses = scrape_upenn()

    data_dir = PROJECT_ROOT / "data"
    data_dir.mkdir(parents=True,exist_ok=True)

    output_path = data_dir/"upenn_courses.json"
